...
import MistickLogo from '@/ui/atoms/MistickLogo'
...
const Header = () => {
  ...
  return (
    <>
      <header
        ref={headerRef}
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out safe-area-top ${isScrolled
        ...
        <div className="container-custom flex items-center justify-between">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className={`font-serif text-base sm:text-lg md:text-xl font-medium tracking-wide leading-none transition-colors duration-300 ${isScrolled ? 'text-mistick-black' : 'text-white'
                }`}>
                Mistick
              </span>
              <span className={`font-sans text-[7px] sm:text-[8px] md:text-[9px] tracking-[0.15em] uppercase leading-tight mt-0.5 transition-colors duration-300 ${isScrolled ? 'text-accent' : 'text-accent-light'
                }`}>
                by Daniele Ribeiro
              </span>
            </div>
          ...
          <button
            ...
            className={`lg:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 transition-colors duration-300 touch-target ${isScrolled ? 'hover:bg-cream-dark/50' : 'hover:bg-white/10'
              }`}
            aria-label={isMenuOpen ? 'Fechar menu' : 'Abrir menu'}
            aria-expanded={isMenuOpen}
          >
            <span
              className={`w-5 h-0.5 transition-all duration-300 origin-center ${isScrolled ? 'bg-mistick-black' : 'bg-white'
                } ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}
            />
            <span
              className={`w-5 h-0.5 transition-all duration-300 ${isScrolled ? 'bg-mistick-black' : 'bg-white'
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`lg:hidden fixed inset-0 bg-mistick-black/60 backdrop-blur-sm z-40 transition-opacity duration-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
      ...
      <div
        ref={menuRef}
        className={`lg:hidden fixed top-0 right-0 h-full w-[280px] sm:w-[320px] bg-white z-50 shadow-2xl transition-transform duration-500 ease-out safe-area-top ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Close button */}

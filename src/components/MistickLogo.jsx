...
export default function MistickLogo({ className = "w-full max-w-lg", delay = 0 }) {
    const containerRef = useRef(null)

    useEffect(() => {
        const ctx = gsap.context(() => {
            // 1. Initial State: Logo paths are invisible (stroke-dasharray setup)
            const paths = containerRef.current.querySelectorAll('path')

            paths.forEach(path => {
                const length = path.getTotalLength()
                // Set up for "drawing" effect: stroke visible, fill invisible
                gsap.set(path, {
                    strokeDasharray: length,
                    strokeDashoffset: length,
                    fillOpacity: 0,
                    stroke: '#E59500', // Amber accent stroke
                    strokeWidth: 2
                })
            })
            ...
            // 3. Fade in the fill and remove stroke
            tl.to(paths, {
                fillOpacity: 1,
                strokeOpacity: 0,
                duration: 1,
    ...
    return (
        <div ref={containerRef} className={className}>
            <svg
                ...
                style={{ color: 'var(--color-mistick-black)' }} // Provide a context color
            >

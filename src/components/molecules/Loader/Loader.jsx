...
import { LOADER_MIN_DURATION_MS } from './Loader.styles'
...
const Loader = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const minLoadTime = setTimeout(() => {
      setIsReady(true)
    }, LOADER_MIN_DURATION_MS) // Min duration to show the animation loop
  ...
  useEffect(() => {
    ...
    gsap.fromTo(textRef.current,
      { y: 15, opacity: 0 },
      { y: 0, opacity: 1, duration: ANIMATION_THEME.duration.sm, ease: ANIMATION_THEME.ease.enter, delay: 0.5 }
    )

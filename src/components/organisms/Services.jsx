...
import { ServiceCard, SectionHeader } from '../molecules'
    ...
    >
      ...
      <div className="container-custom relative z-10">
        <SectionHeader 
          ...
          description="Soluções completas para tornar cada momento especial. Da concepção à realização, estamos ao seu lado."
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6 xl:gap-8">
          {services.map((service, index) => (
            <ServiceCard 
              key={index}
              index={index}
              onRef={el => cardsRef.current[index] = el}
              {...service}
            />
          ))}
        </div>

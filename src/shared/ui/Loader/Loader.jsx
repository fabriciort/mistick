 ...
 * Loader Component
 * 
 * Tela de carregamento com animação do logo Mistick.
 * 
 * @param {Object} props
 * @param {Function} props.onComplete - Callback quando o loading termina
 * @param {number} props.minLoadTime - Tempo mínimo de exibição em ms
 */
const Loader = ({ onComplete, minLoadTime = 2500 }) => {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(minLoadTimeout)
  }, [minLoadTime])
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      progressRef.current,
      { scaleX: 0 },
      { scaleX: 1, duration: minLoadTime / 1000, ease: 'power2.inOut' }
    )
